...
export class RevanthBrain {
  ...
  private initChat() {
    ...
    if (this.pdfContext) {
      systemInstructionText += "\n\n[IMPORTANT]: A PDF document has been uploaded. This PDF will be provided with each message. Use the PDF content as the absolute source of truth for any questions about the information contained in the document.";
    }
  ...
  public async sendMessage(text: string, responseLanguage?: string): Promise<string> {
    if (this.apiKeyMissing || !this.ai) {
      return "Oops! The API key isn't configured. Please create a .env file in the project root with GEMINI_API_KEY=your_api_key and restart the server.";
    }
    ...
    if (!this.chatSession) {
      try {
        ...
        return "Failed to initialize chat session. Please check your API key configuration.";
      }
    }
    ...
    try {
      ...
      // If document exists and hasn't been sent yet, send it as the first message
      if (this.pdfContext && !this.pdfSent) {
        // Note: Gemini API works best with PDFs. For other formats, we'll try to send them
        // but they may need to be converted to PDF for optimal results
        const documentMessageParts: Part[] = [
          { text: "Here is a document (resume/CV or other file). Please read it carefully and use it to answer questions about the information contained in it." },
        ];
        
        // Only include inline data if it's a supported format (PDF or image)
        if (this.pdfContext.mimeType === 'application/pdf' || this.pdfContext.mimeType.startsWith('image/')) {
          documentMessageParts.push({
            inlineData: {
          ...
          // For non-PDF documents, add a note
          documentMessageParts[0].text += " Note: For best results, please convert DOC/DOCX files to PDF format.";
        }
        
        // Send document first to establish context
        await this.chatSession.sendMessage({ message: documentMessageParts });
        this.pdfSent = true;
      }
      ...
      return result.text || "I'm drawing a blank here, can you repeat that?";
    } catch (error: any) {
      console.error("Gemini API Error:", error);
      // Provide more specific error messages
      if (error?.message?.includes('API_KEY') || error?.message?.includes('api key') || error?.message?.includes('API key')) {
        return "Oops! The API key isn't configured properly. Please check the .env file.";
      }
      if (error?.message?.includes('quota') || error?.message?.includes('limit')) {
        return "Looks like I've hit my API limit. Please try again later.";
      }
      if (error?.message?.includes('network') || error?.message?.includes('fetch')) {
        return "Network issue detected. Check your internet connection and try again.";
      }
      return `Something went wrong: ${error?.message || 'Unknown error'}. Try again in a moment.`;
    }
  }
}
